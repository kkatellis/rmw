...
// Prompt will show up after this many seconds
#define FEEDBACK_TIMER_DONTBOTHER      60 * 2
...
// how long to wait before deleting PRE data as irrelevant
#define PRE_DELETE_INTERVAL         45
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (screenHeight==568) {
        musicViewController = [[MusicViewController alloc] initWithNibName:@"MusicViewController5" bundle:nil];
    } else {
        musicViewController = [[MusicViewController alloc] initWithNibName:@"MusicViewController4" bundle:nil];
    }
    ...
    if (screenHeight==568) {
        activityViewController = [[ActivityViewController alloc] initWithNibName:@"ActivityViewController5" bundle:nil];
    } else {
        activityViewController = [[ActivityViewController alloc] initWithNibName:@"ActivityViewController4" bundle:nil];
    }
...
//    NSString *modalClassName = NSStringFromClass([self.window.rootViewController.modalViewController class]);
    
    
    if (activityViewController == self.window.rootViewController.modalViewController) {
//        NSLog(@"parent view controller is rootViewController");
...
//        [self.window.rootViewController presentModalViewController:activityViewController animated:YES];
    } //else if (rootViewController == self.window.rootViewController) {
        //NSLog(@"root view controller");
//    }
    NSLog(@"Start feedback timer - default time");
...
- (void) callExit {
    if (dontBotherStartDate == nil) {
        NSLog(@"Background timer expired, killing app");
        exit(0);
    } else {
        NSLog(@"Background timer expired but don't bother is set, reset timer");
...
//    UIAlertView *feed = [[UIAlertView alloc] initWithTitle: @""
//                                                      message: @"Please provide feedback now!"
//                                                     delegate: self
//                                            cancelButtonTitle: @"Ok"
//                                            otherButtonTitles: nil, nil];
//    [feed show];
...
- (void) promptForFeedback {
    ...
    NSLog( @"Feedback timer went off!" );
    if (feedbackState == kFeedbackFinished){
        NSLog(@"kFeedbackFinished");
    } else if (feedbackState == kFeedbackHidden){
        NSLog(@"kFeedbackHidden");
    } else if (feedbackState == kFeedbackUsing){
        NSLog(@"kFeedbackUsing");
    } else if (feedbackState == kFeedbackWaiting){
        NSLog(@"kFeedbackWaiting");
    } else {
        NSLog(@"Feedback state unknown");
    }
...
//        BOOL userPrompted = FALSE;
        [self performSelector:@selector(localNotif) withObject:nil afterDelay:10];
    }
}
...
- (void) ActivityButtonPressed {
    ...
    BOOL userPrompted = TRUE;
    [self showActivityView:userPrompted];
}
...
//        }
    ...
    if( dontBotherStartDate != nil && !activeFeedback ) {
        NSLog(@"Don't bother amount is set");
        ...
        if( delta < (dontBotherAmount - 10) ) {
            ...
            [sensorController startHFFeedbackSample];
            [sensorController endHFSample];
            [sensorController endDurSoundSample];
            ...
            feedbackState = kFeedbackHidden;
            NSLog(@"start feedback timer - don't bother time");
            feedBackTimer = [NSTimer scheduledTimerWithTimeInterval: FEEDBACK_TIMER_DONTBOTHER
                                                             target: self
                                                           selector: @selector(promptForFeedback)
                                                           userInfo: nil
                                                            repeats: NO];
        ...
        dontBotherAmount    = 0;
        NSLog(@"reset don't bother amount");
        ...
        if (!userPrompted) {
            ...
            NSLog(@"User prompted - don't show local notification");
            [sensorController startHFFeedbackSample];
        }
        // Start feedback sampling
//        [sensorController startHFFeedbackSample];
    ...
    feedBackHider = [NSTimer scheduledTimerWithTimeInterval: FEEDBACK_HIDE_INTERVAL 
                                                   ...
                                                   userInfo: nil 
                                                    repeats: NO];
//    
//    preSampleTimer = [NSTimer scheduledTimerWithTimeInterval: PRE_SAMPLE_TIMER
//                                                     target: self
//                                                   selector: @selector(hideActivityView)
//                                                   userInfo: nil
//                                                    repeats: NO];
...
//        NSLog(@"kFeedbackWaiting");
        ...
        preDeleteTimer = [NSTimer scheduledTimerWithTimeInterval: PRE_DELETE_INTERVAL
                                                         target: sensorController
                                                       selector: @selector(deletePreData)
                                                       userInfo: nil
                                                        repeats: NO];
...
//        [sensorController endDurSoundSample];
    ...
    if( dontBotherStartDate != nil) {
        ...
        // We're still in the dont bother phase, so don't show the feedback view
        if( delta < (dontBotherAmount - 10) ){
            NSLog(@"start feedback timer - don't bother");
            feedBackTimer = [NSTimer scheduledTimerWithTimeInterval: FEEDBACK_TIMER_DONTBOTHER
                                                             target: self
                                                           selector: @selector(promptForFeedback)
                                                           userInfo: nil
            ...
            NSLog(@"start feedback timer - default");
            feedBackTimer = [NSTimer scheduledTimerWithTimeInterval: FEEDBACK_TIMER_DEFAULT
                                                             target: self
                                                           selector: @selector(promptForFeedback)
                                                           userInfo: nil
