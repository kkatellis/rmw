...
//    [soundProcessor startRecording];    // Microphone
    ...
    [NSTimer scheduledTimerWithTimeInterval:SAMPLING_RANGE
                                     target:self
                                  selector :@selector(finishSampling)
                                   userInfo:nil
                                    repeats:NO];
}
...
- (void) _prepStage: (NSString *)fileName {
    ...
    if ([fileName isEqual: HF_PRE_FNAME]){
        NSLog(@"Turning on Pre sound Processor...");
        [soundProcessor startPreRecording];
    }
    else {
        NSLog(@"Turning on Dur sound Processor...");
        [soundProcessor startDurRecording];
    }
...
- (void) _endAndSend {
    ...
    NSLog(@"Turning off dur soundProcessor...");
    [soundProcessor pauseDurRecording];
...
- (void) endPreSoundSample {
    NSLog(@"Turning off Pre soundProcessor...");
    [soundProcessor pausePreRecording];
}

- (void) endDurSoundSample {
    NSLog(@"Turning off Dur soundProcessor...");
    [soundProcessor pauseDurRecording];
}
...
- (void) endHFSample {
    ...
    if (!success) {
        NSLog ( @"UNABLE TO CREATE HF DATA FILE - code: %d - message: %s", errno, strerror(errno) );
    }
...
-(void) packHFData:(NSTimer *)timer{
    ...
    if ([fileName isEqual: HF_PRE_FNAME]){
        // Set microphone data
        [soundProcessor.hfRecorderPre updateMeters];
    
        //--// Record average power for microphone
        NSString *avg = [NSString stringWithFormat:@"%f", [soundProcessor.hfRecorderPre averagePowerForChannel:0]];
        [HFDataList setObject:avg forKey: MIC_AVG];
        
        //--// Record peak power of microphone
        NSString *peak = [NSString stringWithFormat:@"%f", [soundProcessor.hfRecorderPre peakPowerForChannel:0]];
        [HFDataList setObject:peak forKey: MIC_PEAK];
    } else {
        // Set microphone data
        [soundProcessor.hfRecorderDur updateMeters];
        
        //--// Record average power for microphone
        NSString *avg = [NSString stringWithFormat:@"%f", [soundProcessor.hfRecorderDur averagePowerForChannel:0]];
        [HFDataList setObject:avg forKey: MIC_AVG];
        
        //--// Record peak power of microphone
        NSString *peak = [NSString stringWithFormat:@"%f", [soundProcessor.hfRecorderDur peakPowerForChannel:0]];
        [HFDataList setObject:peak forKey: MIC_PEAK];
    }
...
- (void) compressAndSend {
    ...
    NSString *soundFilePathPre = [[dataPath path] stringByAppendingPathComponent: [SoundWaveProcessor hfSoundFileNamePre]];
    NSString *soundFilePathDur = [[dataPath path] stringByAppendingPathComponent: [SoundWaveProcessor hfSoundFileNameDur]];
    ...
    //--// Write the HF_SOUND_PRE file - only if it exists
    if ([[NSFileManager defaultManager] fileExistsAtPath: soundFilePathPre]){
        stream = [zipper writeFileInZipWithName: [SoundWaveProcessor hfSoundFileNamePre]
                               compressionLevel: ZipCompressionLevelFastest];
        
        [stream writeData:[NSData dataWithContentsOfURL:[NSURL fileURLWithPath:soundFilePathPre]]];
        [stream finishedWriting];
    }
    
    //--// Write the HF_SOUND_DUR file
    stream = [zipper writeFileInZipWithName: [SoundWaveProcessor hfSoundFileNameDur]
                           compressionLevel: ZipCompressionLevelFastest];
    
    [stream writeData:[NSData dataWithContentsOfURL:[NSURL fileURLWithPath:soundFilePathDur]]];
    [stream finishedWriting];
    ...
    [fileManager removeItemAtURL:[NSURL fileURLWithPath: soundFilePathDur] error:nil];
    [fileManager removeItemAtURL:[NSURL fileURLWithPath: FBFilePath] error:nil];
    
    // Only delete if the file exists
    if ([fileManager fileExistsAtPath:soundFilePathPre]){
        [fileManager removeItemAtURL:[NSURL fileURLWithPath: soundFilePathPre] error:nil];
    }
...
- (void) deletePreData {
    ...
    NSLog(@"Waited too long - deleting PRE data");
    //--// Get the user's document directory path
    NSURL *dataPath = [DataUploader storagePath];
    assert( dataPath );
